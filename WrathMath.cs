...
namespace ACT_Plugin
{
     ...
     */
    public class WrathMath : IActPluginV1
    {
#if DEBUG
...
<body>
    ...
    <p>
        ...
        <em>/do_file_commands C:\EQ2\WrathMathCommands.txt</em>.
    </p>
    <h2>Help!</h2>
    <p>Send an in-game tell to Skyfire.Reapp, or on Discord (samo#7395) and I'll try to help out if I can!</p>
        ...
        void IActPluginV1.InitPlugin(TabPage pluginScreenSpace, Label pluginStatusText)
        {
            ...
            var webHelp = new WebBrowser()
            {
                DocumentText = HELP_TEXT,
                Dock = DockStyle.Fill
            };
            ...
            _hud = new WrathMathHud(_settings);
            _hud.FormClosing += hud_FormClosing;
        ...
        private void OFormActMain_OnCombatStart(bool isImport, CombatToggleEventArgs encounterInfo)
        {
            ...
            // parse time during the intended mob, and ensures we don't capture player dets for mobs
            // other than the one we want (eg. second-last in VD2).
            _zoneDetected = false;
            _mobDetected = false;
        }
        ...
        private void OFormActMain_OnLogLineRead(bool isImport, LogLineEventArgs logInfo)
        {
            ...
            if (DEBUG || (!isImport && logInfo.inCombat))
            {
                ...
                {
                    if (!_mobDetected && ActGlobals.oFormActMain.ActiveZone.ActiveEncounter != null)
                    {
                        // @Aditu - is there a better way to detect the current mob?
                        foreach (var combatant in ActGlobals.oFormActMain.ActiveZone.ActiveEncounter.Items)
                        {
                            if (combatant.Key.Equals("THE ABANDONED LABOMINATION")) // Keys are always uppercase
                            {
                                _mobDetected = true;
                                break;
                            }
